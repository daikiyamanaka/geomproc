...
void TriangleMesh::getCentroids(std::vector<Eigen::Vector3d> &_centroids) const
{
...
void TriangleMesh::getBBox(Eigen::Vector3d &min, Eigen::Vector3d &max){
...
Eigen::Vector3d TriangleMesh::getCenter(){
...
void TriangleMesh::initialize(){
   ...
   //1-ring neighbor faces
    ...
    v_neighbors.resize(num_of_vertices);    
    for(int i=0; i<num_of_vertices; i++){
       std::vector<int> v_neighbor;
        for(int j=0; j<ringN[i]; j++){
            for(int k=0; k<3; k++){
                int v_index = facets[ring[i][j]][k];
                if(v_index == i) 
                    continue;

                if(std::find(v_neighbor.begin(), v_neighbor.end(), v_index) == v_neighbor.end()){
                    v_neighbor.push_back(v_index);
                }
            }
        }
        v_neighbors[i] = v_neighbor;        
    }
    ...
    for(int i=0; i<num_of_faces; i++){        
        f_normals[i] = (points[facets[i][1]]-points[facets[i][0]]).cross(points[facets[i][2]]-points[facets[i][0]]);
        f_normals[i].normalize();
    }

    // vertex normals
    v_normals.resize(num_of_vertices);
    for(int i=0; i<num_of_vertices; i++){
      Eigen::Vector3d v_normal = Eigen::Vector3d::Zero();
      for(int j=0; j<(int)v_f_map[i].size(); j++){
          v_normal += f_normals[v_f_map[i][j]];
      }
      v_normal.normalize();
      v_normals[i] = v_normal;
    }
